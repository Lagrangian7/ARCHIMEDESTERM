...
ONLY output the code, no explanations.`;
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      res.cookie('custom_session', sessionToken, {
        httpOnly: true,
        secure: false,
  ...
  app.post('/api/auth/magic-link', async (req, res) => {
    try {
      ...
      // Store token (use Redis or DB in production)
      (req.session as any).magicToken = { token, email, expires };
      
      // Send email (integrate with email service)
      const link = `${req.protocol}://${req.hostname}/api/auth/verify?token=${token}`;
  ...
  app.get('/api/auth/verify', async (req, res) => {
    try {
      ...
      const magicData = (req.session as any).magicToken;
      
      if (!magicData || magicData.token !== token || new Date() > magicData.expires) {
        return res.redirect('/?error=invalid_token');
      }
      
      // Get or create user
      let user = await storage.getUserByEmail(magicData.email);
      if (!user) {
      ...
      // Create session
      (req.session as any).userId = user.id;
      delete (req.session as any).magicToken;
