...
`;
  ...
  app.post("/api/chat/stream", async (req, res) => {
    try {
      ...
      // Build enhanced message for freestyle mode
      let enhancedMessage = message;
      if (mode === 'freestyle') {
        const contextParts: string[] = [];
        if (programmingLanguage) {
        ...
        if (editorContext && typeof editorContext === 'string' && editorContext.trim()) {
          const truncatedContext = editorContext.length > 1000 
            ? editorContext.substring(0, 1000) + '\n...[truncated]'
            : editorContext;
          contextParts.push(`[Current Editor Content]:\n\`\`\`\n${truncatedContext}\n\`\`\``);
        }
        if (contextParts.length > 0) {
          enhancedMessage = contextParts.join('\n') + '\n\n' + message;
        }
      ...
      res.write(`data: ${JSON.stringify({ type: 'start', sessionId: currentSessionId })}\n\n`);

      let fullResponse = '';

      try {
        // Stream the response
        const streamGenerator = llmService.generateStreamingResponse(
          enhancedMessage,
          mode as 'natural' | 'technical' | 'freestyle' | 'health',
          conversationHistory,
          user.id,
          language || 'english',
          isNewSession
        ...
        // Post-process code in freestyle mode
        if (mode === 'freestyle') {
          fullResponse = cleanupGeneratedCode(fullResponse);
        }
        ...
        res.write(`data: ${JSON.stringify({ type: 'done', fullResponse, sessionId: currentSessionId })}\n\n`);
        res.end();
